...
# Full Release Pipeline for slaygent-comms
# Handles: commit, push, tag, homebrew update, and GitHub release with changelog
# Adapted for multi-binary build structure (msg, slaygent-manager, msg-ssh)
...
# Configuration
REPO_ORG="WillyV3"
REPO_NAME="slaygent-comms"
HOMEBREW_TAP_PATH="$HOME/homebrew-tap"
FORMULA_FILE="$HOMEBREW_TAP_PATH/Formula/slaygent-comms.rb"
...
main() {
    echo ""
    echo "========================================="
    echo "  slaygent-comms Full Release Pipeline"
    echo "========================================="
    echo ""
    ...
    if go test ./... > /dev/null 2>&1; then
        print_success "Messenger tests passed"
    else
        print_warning "Messenger tests failed, continuing anyway"
        TEST_PASSED=false
    fi
    cd ../..
    ...
    cd app/tui
    if go test ./... > /dev/null 2>&1; then
        print_success "TUI tests passed"
    else
        print_warning "TUI tests failed, continuing anyway"
        TEST_PASSED=false
    fi
    cd ../..
    ...
    cd app/msg-ssh
    if go test ./... > /dev/null 2>&1; then
        print_success "msg-ssh tests passed"
    else
        print_warning "msg-ssh tests failed, continuing anyway"
        TEST_PASSED=false
    fi
    cd ../..
    ...
    # Build messenger (msg)
    cd app/messenger
    if go build -o msg .; then
        print_success "Built msg binary"
        rm -f msg
    else
        print_error "Failed to build msg binary"
    fi
    cd ../..

    # Build TUI (slaygent-manager)
    cd app/tui
    if go build -o slaygent-manager .; then
        print_success "Built slaygent-manager binary"
        rm -f slaygent-manager
    else
        print_error "Failed to build slaygent-manager binary"
    fi
    cd ../..

    # Build msg-ssh
    cd app/msg-ssh
    if go build -o msg-ssh .; then
        print_success "Built msg-ssh binary"
        rm -f msg-ssh
    else
        print_error "Failed to build msg-ssh binary"
    fi
    cd ../..
    ...
    if [[ -f $FORMULA_FILE ]]; then
        ...
        git add Formula/slaygent-comms.rb
        git commit -m "Update slaygent-comms to ${NEW_VERSION}"
        git push
        cd - > /dev/null
    ...
    echo ""
    echo "Installation:"
    echo "  brew upgrade slaygent-comms"
    echo "  or"
    echo "  brew install ${REPO_ORG}/tap/slaygent-comms"
    echo ""
    echo "Release URL:"
